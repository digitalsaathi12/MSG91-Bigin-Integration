...
from app.services.bigin_client import BiginClient, BiginAPIException
...
@router.post(
    ...
    description="Receives MSG91 WhatsApp webhook payload, validates URL path secret, and immediately calls Bigin REST API to create the Lead."
)
def msg91_webhook_pass_through(
    ...
    try:
        ...
        logger.info(f"Successfully created Bigin Lead ID '{bigin_lead_id}' for phone '{payload.phone}'.")
        ...
        raise HTTPException(
            ...
            detail=f"Bigin API sync error: {str(err)}"
        )
    except Exception as err:
        logger.error(f"Unexpected error processing lead for phone '{payload.phone}': {err}")
        raise HTTPException(
